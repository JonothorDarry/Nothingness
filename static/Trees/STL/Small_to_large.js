...
class Small_to_large extends Algorithm{
	...
	presentation_append_companion(vertex, x, y, name, color=5, width_height=30){
	...
	presentation_add_set(){
		...
		for (var i=1; i<=this.logic.tree.n; i++){
			var set = this.presentation_append_companion(i, 1, 1, '', 31, 2*this.presentation.set_radius);
	...
	StateMaker(s){
		...
		if (s[0]==0){
			for (var i=1; i<=this.logic.tree.n; i++){
				staat.push([7, this.buttons.set_elements[i][this.logic.tree.colors[i]].color, {'background':this._presentation.color_mapping[this.logic.tree.colors[i]], 'color':'#FFFFFF'}]);
		...
		if (s[0] == 2){
			...
			if (operation[2]){
				...
				for (var x of operation[0]) staat.push([7, this.buttons.set_elements[parental][x].color, {'background':this._presentation.color_mapping[x], 'color':'#FFFFFF'}]);
				for (var x of operation[1]) staat.push([7, this.buttons.set_elements[vertex][x].color, {'background':this._presentation.color_mapping[x], 'color':'#FFFFFF'}]);
			}
			else {
		...
		if (s[0]==3){
			...
			staat.push([7, this.buttons.set_elements[this.logic.tree.par[vertex]][next_element].color, {'background':this._presentation.color_mapping[next_element], 'color':'#FFFFFF'}]);
