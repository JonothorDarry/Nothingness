class Isomorphic_rooted extends Algorithm{
	...
	presentation(){
		...
		var siege_place=elems.zdivs, dh='90px', i, j, ij;
		...
		var number_width=20, tree_width=20, div_width=84; //Change adequately later

		for (i=0; i<=this.logic.n; i++){
			elems.divs[i].style.height=dh;
			elems.divs[i].style.position='relative';

			for (j=0; j<=i; j++){
				//Tutaj nie dociera drzewo
				var bad_place=-1;
				var div=this.div_creator({'bs_div_width':`${div_width}px`, 'bs_div_height':'84px', 'bs_div_borderSize':'3px'});
				...
				for (ij=0; ij<all_alocated_trees; ij++){
					var post_div=this.div_creator({'bs_div_width':`${tree_width}px`, 'bs_div_height':'20px', 'bs_div_borderSize':'0px'});
